...
function preload() {
    ...
    */
    ...
    var logoSheet = loadSpriteSheet(ASSETS_FOLDER + LOGO_FILE, 128, 100, 1);
...
function muteSound(){
	if(mute== false){
		masterVolume(0);
		mute= true;
		document.getElementById("mute-btn").innerHTML= "ACTIVAR SONIDO";
	}else{
		masterVolume(0.3);
		mute= false;
		document.getElementById("mute-btn").innerHTML= "SILENCIAR";
	}
    ...
    socket.on("playerJoined",
        function (p) {
            try {
                ...
                if (socket.id == p.id) {
                    ...
                    */
                    ...
                    //music is synched across clients
						console.log("SAME MUSIC: "+sameMusic);
                    if (ROOMS[p.room].musicLoop != null && SOUND && sameMusic == false) {
                ...
                console.log("There are now " + Object.keys(players).length + " players in this room", p.x, p.y);
...
function update() {
    ...
    //renders the avatar selection screen which can be fully within the canvas
        ...
        text("Elige tu avatar", 64 * ASSET_SCALE, 18 * ASSET_SCALE);
...
function canvasReleased() {
    ...
    if (screen == "error") {
        ...
        if (longText != "" && longText != SETTINGS.INTRO_TEXT) {
	        ...
	        if (longText != "" && longText != SETTINGS.INTRO_TEXT) {
				if(longTextLink=="video1"){
					masterVolume(0);
					 video.innerHTML = '<div style="padding:56.25% 0 0 0;position:relative;"><iframe src="https://player.vimeo.com/video/570980129?autoplay=1&color=ffffff&title=0&byline=0&portrait=0" style="position:absolute;top:0;left:0;width:100%;height:100%;" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe></div><script src="https://player.vimeo.com/api/player.js"></script>';
				   modal.style.display = "block";
						 		
					
				}else if(longTextLink=="video2"){
					masterVolume(0);
					 video.innerHTML ='<div style="padding:56.25% 0 0 0;position:relative;"><iframe src="https://player.vimeo.com/video/570980229?autoplay=1&color=ffffff&title=0&byline=0&portrait=0" style="position:absolute;top:0;left:0;width:100%;height:100%;" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe></div><script src="https://player.vimeo.com/api/player.js"></script>';
					 modal.style.display = "block";
				}else if(longTextLink=="video3"){
					masterVolume(0);
					video.innerHTML ='<div style="padding:56.25% 0 0 0;position:relative;"><iframe src="https://player.vimeo.com/video/570980466?autoplay=1&color=ffffff&title=0&byline=0&portrait=0" style="position:absolute;top:0;left:0;width:100%;height:100%;" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe></div><script src="https://player.vimeo.com/api/player.js"></script>';
					modal.style.display = "block";
				
				}else if(longTextLink=="video4"){
					masterVolume(0);
        ...
        case "enter":
            ...
            if (ROOMS[c.room] != null) {
                ...
                if (ROOMS[me.room].musicLoop != null) {
					...
					if(ROOMS[me.room].musicLoop.url!=ROOMS[c.room].musicLoop.url){
					...
					console.log("LOG:stops music");
					
                }
				console.log("ENTER POINT = "+c.enterPoint );
                if (c.enterPoint != null) {
					...
					console.log("TRANSFORM ENTER POINT = "+sx,sy,ASSET_SCALE);
                }
					...
					console.log("SPAWN ENTER POINT = "+sx,sy,ASSET_SCALE);
                }
                else {
...
function nameValidationCallBack(code) {
    if (socket.id) {
        ...
        if (code == 0) {
            ...
            if (e != null)
                e.innerHTML = "Lo siento, no se permiten caracteres especiales";
        }
        else {
